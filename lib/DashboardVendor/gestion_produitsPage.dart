...
class _GestionProduitsPageState extends State<GestionProduitsPage> {
  ...
  void _navigateToAddProduit() async {
    final result = await Navigator.of(context).push(
      MaterialPageRoute(builder: (context) => const AjoutProduitsPage()),
    );
  ...
  void _showAdminWarning() {
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(
        content: Text(
          'Action interdite. Veuillez contacter l’administrateur.',
                    ...
                    rows: _filteredProduits.map((produit) {
                      ...
                      return DataRow(
                        ...
                        cells: [
                          ...
                          // Actions désactivées pour le vendeur
                          DataCell(Row(
                            children: [
                              IconButton(
                                icon: Icon(Icons.edit,
                                    color: Colors.grey.shade400, size: 20),
                                onPressed: _showAdminWarning,
                              ),
                              IconButton(
